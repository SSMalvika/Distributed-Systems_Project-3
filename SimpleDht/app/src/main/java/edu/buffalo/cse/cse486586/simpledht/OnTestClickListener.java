...
public class OnTestClickListener implements OnClickListener {
	...
	private class Task extends AsyncTask<Void, String, Void> {
		...
		protected Void doInBackground(Void... params) {
			...
			if (testqueryat()) {
				publishProgress("Delete @ has been a success\n");
			} else {
				publishProgress("Delete @ has been a fail\n");
			}
		...
		private boolean testquerystar() {
			try {
				...
				Cursor resultCursor= mContentResolver.query(mUri, null,
						"*", null, null);
				Log.i("test",Integer.toString(resultCursor.getCount()));
		...
		private boolean testDeleteStar() {
			try {
			...
			try {
				for (int i = 0; i < TEST_CNT; i++) {
					...
					if (!(resultCursor.isFirst() && resultCursor.isLast())) {
						Log.e(TAG,Integer.toString(resultCursor.getCount()));
